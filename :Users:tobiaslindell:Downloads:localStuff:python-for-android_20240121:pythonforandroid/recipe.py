...
class Recipe(metaclass=RecipeMeta):
    ...
    def get_build_dir(self, arch):
        ...
        #print(f"buid_dir: {join(self.get_build_container_dir(arch), self.name)}")
    ...
    def get_recipe_dir(self):
        ...
        if self.ctx.local_recipes is not None:
            ...
            if exists(local_recipe_dir):
                #print(f"local_recipe_dir: {local_recipe_dir}")
    ...
    def install_libraries(self, arch):
        ...
        #print(f"shared_libs: {shared_libs}")
    ...
    def install_python_package(self, arch, name=None, env=None, is_dir=True):
        ...
        if self.name == "cryptography":
            with current_directory(self.get_build_dir(arch.arch)):
                ...
                shprint(sh.Command('pip'), 'install', '.', '--target=/Users/tobiaslindell/appStuff/.buildozer/android/platform/build-arm64-v8a/build/python-installs/convokeeper/arm64-v8a', _env=env)
                '''
...
class CompiledComponentsPythonRecipe(PythonRecipe):
    ...
    def build_compiled_components(self, arch):
        ...
        #print("def build_compiled_components")
        ...
        if self.name == "cryptography":
            with current_directory(self.get_build_dir(arch.arch)):
                if self.install_in_hostpython:
                    shprint(hostpython, 'setup.py', 'clean', '--all', _env=env)
                shprint(sh.Command('pip'), 'install', '.', '--target=/Users/tobiaslindell/appStuff/.buildozer/android/platform/build-arm64-v8a/build/python-installs/convokeeper/arm64-v8a', _env=env)
                build_dir = "/Users/tobiaslindell/appStuff/.buildozer/android/platform/build-arm64-v8a/build/python-installs/convokeeper/arm64-v8a"
                #build_dir = glob.glob('build/lib.*')[0]
                shprint(sh.find, build_dir, '-name', '"*.o"', '-exec',
                        env['STRIP'], '{}', ';', _env=env)
