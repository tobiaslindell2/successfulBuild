...
class OpenSSLPrerequisite(Prerequisite):
    name = "openssl"
    homebrew_formula_name = "openssl@1.1"  # This can be kept for reference
    mandatory = dict(linux=False, darwin=True)
    installer_is_supported = dict(linux=False, darwin=True)

    def darwin_checker(self):
        # Check if OpenSSL is installed in /usr/local/openssl-1.1
        #openssl_lib_path = "/usr/local/openssl-1.1/lib"
        openssl_lib_path = "/Users/tobiaslindell/Downloads/localStuff/openssl1.1"
        openssl_include_path = "/Users/tobiaslindell/Downloads/localStuff/openssl1.1/include"
        #openssl_include_path = "/usr/local/openssl-1.1/include"
        return os.path.isdir(openssl_lib_path) and os.path.isdir(openssl_include_path)

    def darwin_pkg_config_location(self):
        # Return the pkg-config path for the local OpenSSL installation
        return "/usr/local/openssl-1.1/lib/pkgconfig"

    def darwin_installer(self):
        # Optionally, you can implement a custom installer or log a message
        info("Using local OpenSSL installation at /usr/local/openssl-1.1")
